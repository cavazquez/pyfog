...
class PluginDescriptor(Schema):
    ...
    def valid_executable(cls, value: list[str]) -> list[str]:
        if any(
            ...
            msg = "El comando del plugin contiene un argumento inválido."
    ...
    def valid_names(cls, value: list[str]) -> list[str]:
        if len(set(value)) != len(value) or any(
            ...
            msg = "Las capacidades y permisos del plugin no son válidos."
            raise ValueError(msg)
        return sorted(value)
    ...
    def valid_environment(cls, value: list[str]) -> list[str]:
        if len(set(value)) != len(value) or any(
            ...
            msg = "El plugin sólo puede declarar variables PYFOG_PLUGIN_*."
            raise ValueError(msg)
        return sorted(value)
    ...
    def validate_capabilities(self) -> PluginDescriptor:
        if not self.capabilities:
            msg = "El plugin debe declarar al menos una capacidad."
            raise ValueError(msg)
        if any(permission not in self.capabilities for permission in self.permissions):
            msg = "Un permiso del plugin debe estar cubierto por una capacidad."
            raise ValueError(msg)
        return self
...
class PluginResponse(Schema):
    ...
    def validate_result(self) -> PluginResponse:
        if self.success and self.error:
            msg = "Una respuesta exitosa no puede contener un error."
            raise ValueError(msg)
        if not self.success and not self.error:
            msg = "Una respuesta fallida debe explicar el error."
            raise ValueError(msg)
    ...
    if isinstance(value, Mapping):
        for key, item in value.items():
            ...
            if (
                ...
                msg = f"El payload del plugin no puede contener {path}.{key}."
...
def _json_bytes(value: Mapping[str, Any]) -> bytes:
    try:
        ...
        msg = f"El payload del plugin no es JSON: {error}"
        raise PluginError(msg) from None
...
class PluginRegistry:
    ...
    def register(self, descriptor: PluginDescriptor) -> None:
        if descriptor.name in self._descriptors:
            msg = f"El plugin {descriptor.name} ya está registrado."
            raise PluginError(msg)
    ...
    def get(self, name: str) -> PluginDescriptor:
        try:
            ...
            msg = f"No existe el plugin {name}."
            raise PluginError(msg) from None
    ...
    def load_file(self, path: Path) -> PluginDescriptor:
        if path.is_symlink() or not path.is_file():
            msg = "El descriptor del plugin debe ser un archivo regular."
            raise PluginError(msg)
        try:
            value = json.loads(path.read_text(encoding="utf-8"))
            descriptor = PluginDescriptor.model_validate(value)
        except (OSError, UnicodeError, json.JSONDecodeError, ValueError) as error:
            msg = f"El descriptor del plugin es inválido: {error}"
            raise PluginError(msg) from None
    ...
    def load_directory(self, directory: Path) -> tuple[PluginDescriptor, ...]:
        if directory.is_symlink() or not directory.is_dir():
            msg = "El directorio de plugins debe ser un directorio regular."
            raise PluginError(msg)
    ...
    def run(self, plugin_name: str, call: PluginCall) -> PluginResponse:
        ...
        if call.secret is not None:
            if not isinstance(call.secret, bytes) or not 1 <= len(call.secret) <= 16 * 1024:
                msg = "El secreto del plugin no tiene un tamaño permitido."
                raise PluginError(msg)
        ...
        if len(encoded) > descriptor.max_input_bytes:
            msg = "La solicitud del plugin supera el límite de entrada."
            raise PluginError(msg)
        ...
        try:
            ...
            msg = f"El plugin {plugin_name} no terminó dentro del límite: {error}"
            raise PluginError(msg) from None
        if len(completed.stdout) > descriptor.max_output_bytes:
            msg = "La respuesta del plugin supera el límite de salida."
            raise PluginError(msg)
        if completed.returncode != 0:
            msg = f"El plugin {plugin_name} terminó con código {completed.returncode}."
            raise PluginError(msg)
        try:
            value = json.loads(completed.stdout)
            response = PluginResponse.model_validate(value)
        except (UnicodeDecodeError, json.JSONDecodeError, ValueError) as error:
            msg = f"El plugin {plugin_name} devolvió un contrato inválido: {error}"
            raise PluginError(msg) from None
        if response.request_id != call.request_id:
            msg = "La respuesta del plugin no corresponde a la solicitud."
            raise PluginError(msg)
        return response
    ...
    def _resolve_command(descriptor: PluginDescriptor) -> list[str]:
        ...
        if "/" in executable:
            ...
            if not resolved_path.is_file() or not os.access(resolved_path, os.X_OK):
                msg = "El ejecutable del plugin no es un archivo ejecutable seguro."
                raise PluginError(msg)
            resolved = str(resolved_path)
        else:
            resolved = shutil.which(executable) or ""
            if not resolved:
                msg = f"No se encontró el ejecutable del plugin: {executable}."
                raise PluginError(msg)
